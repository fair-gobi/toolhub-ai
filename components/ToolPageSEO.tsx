...
export function ToolPageSEO({ name: propName, cat: propCat, path, slug }: Props) {
  ...
  useEffect(()=>{
    fetch(`/api/tool-articles/${effectiveSlug}`).then(r=>r.json()).then(d=>{ if(d) setArticle(d) }).catch(()=>{})
  },[effectiveSlug])
  ...
  return (
    <section className="max-w-4xl mx-auto mt-12 px-6 py-10 bg-white dark:bg-gray-900 border rounded-2xl">
      <article className="prose dark:prose-invert max-w-none">
        <h2>What is {name}?</h2>
        <p>{intro || `${name} is a free online ${cat.toLowerCase()} tool from PromptoolHub that helps you solve tasks instantly. No signup, private, fast.`}</p>

        <h2>How to Use {name}?</h2>
        {steps?.length? <ol>{steps.map((s,i)=><li key={i}>{s}</li>)}</ol> : <ol><li>Open promptoolhub.com{path}</li><li>Enter your input</li><li>Click Generate</li><li>Copy or Download instantly</li></ol>}

        {logic && <><h2>How {name} Works?</h2><p>{logic}</p></>}

        <h2>Key Features</h2>
        {features?.length? <ul>{features.map((f,i)=><li key={i}>{f}</li>)}</ul> : <ul><li>100% Free & Unlimited</li><li>Privacy First</li><li>Works on mobile</li><li>Fast & Accurate</li><li>No Signup</li></ul>}

        <h2>FAQs - {name}</h2>
        <div className="mb-3"><h3 className="font-semibold">Is {name} free?</h3><p>Yes, 100% free, no signup.</p></div>
        <div className="mb-3"><h3 className="font-semibold">Is my data safe?</h3><p>Processing in browser/serverless, auto-deleted.</p></div>
      </article>
