...
export function ToolPageSEO({ name, cat, path }: Props) {
  ...
  const softwareSchema = {
    ...
    "description": `${name} - free ${cat.toLowerCase()} tool`,
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": "Home", "item": "https://www.promptoolhub.com" },
      { "@type": "ListItem", "position": 2, "name": cat, "item": `https://www.promptoolhub.com/${cat.toLowerCase().replace(' ', '-')}` },
      { "@type": "ListItem", "position": 3, "name": name, "item": `https://www.promptoolhub.com${path}` },
    ]
  }
    ...
    <>
      ...
      <section className="max-w-4xl mx-auto mt-12 px-6 py-10 bg-white dark:bg-gray-900 border rounded-2xl">
        <article className="prose dark:prose-invert max-w-none">
          <h2>What is {name}?</h2>
          <p><strong>{name}</strong> is a free online {cat.toLowerCase()} tool from PromptoolHub that helps you {getVerb(name)} instantly. No signup, private, fast.</p>
