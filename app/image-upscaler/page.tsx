  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-2xl mx-auto">
        <Link href="/">← Back</Link>
        <h1 className="text-3xl font-bold mt-4">Image Upscaler 4x</h1>
        <div className="bg-white p-6 rounded-xl shadow mt-4">
          <input type="file" accept="image/*" onChange={upscale} />
          {loading && <p>Upscaling...</p>}
          {img && <img src={img} className="mt-4 max-w-xs" />}
          {out && <><img src={out} className="mt-4" /><a href={out} download="upscaled.png" className="block mt-2 bg-blue-600 text-white py-2 text-center rounded">Download</a></>}
        </div>
      </div>
    </div>
  )
