  ...
  const handleScan = (data: string) => {
    ...
    if (clean.toLowerCase().startsWith('otpauth://')) {
      try {
        ...
        const totp = new OTPAuth.TOTP({ secret: sec })
        setOtp(totp.generate())
      } catch {
        setIs2FA(false)
      }
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-gray-600 mb-6">Scan QR codes • Generate 2FA codes offline</p>

        <div className="bg-white p-6 rounded-2xl border shadow-sm">
          {!scanning &&!result && (
            <div className="space-y-3">
              <button onClick={startCamera} className="w-full bg-purple-600 hover:bg-purple-700 text-white py-4 rounded-xl font-medium transition">📷 Scan with Camera</button>
          ...
          {scanning && (
            <div>
              ...
              <button onClick={stopCamera} className="w-full bg-red-600 hover:bg-red-700 text-white py-3 rounded-lg font-medium">Stop Camera</button>
            </div>
          )}
              ...
              {is2FA? (
                <div className="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border border-blue-200">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <div className="font-semibold text-lg">{issuer}</div>
                      <div className="text-sm text-gray-600">{account}</div>
                    </div>
                  </div>

                  <div className="bg-white p-6 rounded-xl text-center mb-4 shadow-sm">
                    ...
                    <div className="mt-4">
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div className="bg-blue-600 h-2 rounded-full transition-all duration-1000" style={{ width: `${(timeLeft / 30) * 100}%` }}></div>
                      </div>
                      <div className="text-xs text-gray-500 mt-1.5">Refreshes in {timeLeft}s</div>
                    </div>
                  </div>

                  <div className="bg-white/60 backdrop-blur p-3 rounded-lg mb-4">
                  ...
                  <button onClick={() => { setResult(''); setIs2FA(false) }} className="w-full mt-3 text-sm text-gray-600 hover:text-gray-900 py-2">← Scan Another</button>
                </div>
              ) : (
                <div className="p-5 bg-green-50 rounded-2xl border border-green-200">
                  ...
                  <div className="font-mono break-all bg-white p-4 rounded-xl border text-sm">{result}</div>
                  <div className="mt-4 flex gap-2.5">
                    <button onClick={() => navigator.clipboard.writeText(result)} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-lg text-sm font-medium">Copy</button>
                    {result.startsWith('http') && <a href={result} target="_blank" rel="noopener" className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2.5 rounded-lg text-sm font-medium text-center">Open Link</a>}
                  </div>
                  <button onClick={() => setResult('')} className="w-full mt-2.5 text-sm text-gray-600 hover:text-gray-900 py-2">Scan Again</button>
                </div>
              )}
            </div>
          )}
        ...
        <div className="mt-6 text-center">
          <p className="text-xs text-gray-500">✓ 100% private • All processing in your browser • No data sent to servers</p>
        </div>
      </div>
    </main>
  )
