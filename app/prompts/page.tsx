...
export default async function PromptsPage({ searchParams }: { searchParams: Promise<{ cat?: string, q?: string, page?: string }> }) {
  ...
  if (active.db === 'Hero') {
    ...
    prompts = await sql`SELECT id, title, slug, category, prompt_content, seo_description, tags, is_hero FROM prompts WHERE category=${active.db} AND (${q} = '' OR title ILIKE ${'%' + q + '%'}) ORDER BY is_hero DESC, id DESC LIMIT ${perPage} OFFSET ${offset}`
    const [{count: c}] = await sql`SELECT COUNT(*) FROM prompts WHERE category=${active.db} AND (${q} = '' OR title ILIKE ${'%' + q + '%'})`
    count = Number(c)
  } else {
    prompts = await sql`SELECT id, title, slug, category, prompt_content, seo_description, tags, is_hero FROM prompts WHERE ${q} = '' OR title ILIKE ${'%' + q + '%'} ORDER BY is_hero DESC, id DESC LIMIT ${perPage} OFFSET ${offset}`
    const [{count: c}] = await sql`SELECT COUNT(*) FROM prompts WHERE ${q} = '' OR title ILIKE ${'%' + q + '%'}`
    count = Number(c)
  }
        ...
        <div className="mx-auto max-w- px-4 md:px-6 h- flex items-center justify-between">
          ...
          <form method="GET" action="/prompts" className="hidden md:flex items-center gap-2">
            ...
            <div className="relative">
              <input name="q" defaultValue={q} placeholder="Search prompts, e.g. logo, SaaS..." className="w- h-9 pl-9 pr-3 rounded-full bg-zinc-100 border border-zinc-200 text-sm focus:outline-none focus:bg-white focus:border-black" />
      ...
      <div className="mx-auto max-w- px-4 md:px-6 py-6 md:py-8">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-zinc-500 text-sm md:text- mt-2">Production-ready prompts for ChatGPT, Claude, Midjourney, Runway. Copy → Try → Ship.</p>
          </div>
          <div className="text-xs text-zinc-500">Page {page} of {totalPages} • {count.toLocaleString()} results {q && `for "${q}"`}</div>
        </div>

        {/* Pills - scrollable on mobile */}
        <div className="mt-6 -mx-4 md:mx-0 px-4 md:px-0 overflow-x-auto scrollbar-none">
          <div className="flex gap-2 w-max">
            {CATEGORIES.map(c=>(
              <Link key={c.key} href={`/prompts?cat=${c.key}${q? `&q=${encodeURIComponent(q)}`:''}`}
                className={`h-8 px-4 rounded-full text- font-semibold border whitespace-nowrap transition flex items-center gap-1.5 ${activeKey===c.key? 'bg-black text-white border-black' : 'bg-white border-zinc-200 hover:border-black hover:text-black text-zinc-600'}`}>
            ...
            return (
              <div key={p.id} className={`group relative bg-white rounded- border p-5 flex flex-col gap-3 hover:shadow-xl hover:border-zinc-900 transition-all ${p.is_hero? 'border-amber-200 ring-1 ring-amber-100' : 'border-zinc-200'}`}>
                ...
                <Link href={`/prompts/${p.slug}`} className="space-y-2">
                  <h3 className="font-semibold text- leading-[1.25] line-clamp-2 group-hover:text-violet-600 transition">{p.title}</h3>
        ...
        {/* Empty */}
        {prompts.length===0 && <div className="py-20 text-center text-zinc-500">No prompts found for "{q}" in {active.label}</div>}
