    ...
    <main className="container mx-auto p-6 max-w-4xl">
      <div className="bg-gradient-to-r from-pink-600 to-rose-500 text-white rounded-xl p-6 mb-6 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="opacity-90">Create client quotes — print to PDF</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border rounded-xl p-8 mt-6">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((it,i)=>(
              <tr key={i} className="border-b"><td className="py-2">{it.desc}</td><td className="text-right">{it.qty}</td><td className="text-right">${it.price}</td><td className="text-right">${(it.qty*it.price).toFixed(2)}</td></tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="flex justify-end mt-4">
          <div className="text-right">
            ...
            <p className="text-xs text-gray-500 mt-2">Terms: 50% advance, balance on delivery</p>
          </div>
        </div>
      </div>
